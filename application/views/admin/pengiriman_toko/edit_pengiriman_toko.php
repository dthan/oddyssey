          ...
          <div class="row-fluid">
            <div class="span12">
              ...
              <ul class="breadcrumb br_styled no_space">
                ...
                <li class="active">edit toko</li>
              </ul> 
              <ul class="page_info" >          
                     <?php
                    foreach ($pengiriman_toko as $k) {  }             
                    ?>       
                  <form  method="POST" id="form_input_toko"  action="<?= base_url() ?>pengiriman_toko/aksi_edit">                     
                    
                   <div class="row-fluid">
                        <div class="span3">Nama toko:</div>
                        <div class="span9"><?= form_dropdown('toko', $toko,  $k->id_toko, 'id="toko" required class="chzn-select" data-placeholder="pilih toko"  style="width:220px;" tabindex="2" ') ?>
                       ...
                       <div class="row-fluid">
                        ...
                        <input  type="checkbox" name="samakan"  id="samakan" >samakan dengan alamat toko<br/>
                             <textarea id="alamat" required="" name="alamat"><?= $k->alamat_penerima ?></textarea></div> 
                      </div> 
                       <div class="row-fluid">
                        <div class="span3">Kurir</div>
                        <div class="span9"><?= form_dropdown('jasa_pengiriman', $jasa_pengiriman,  $k->jasa_pengiriman, 'id="kurir" required class="chzn-select" data-placeholder="pilih kurir"  style="width:220px;" tabindex="2" ') ?></div> 
                      </div>  
                     
                      <div class="row-fluid">
                        <div class="span3">Biaya Pengiriman:</div>
                         <div class="span9"><input type="text" id="biaya_pengiriman" value="<?= $k->biaya_pengiriman ?>" name="biaya_pengiriman">
                         </div> 
                      </div> 
                       <?php
                       $i=1;
                       foreach ($detail_pengiriman as $h) {
                     
                        ?>
                        <div class="row-fluid" id="produk<?= $i ?>" >
                         <div class="span3"><?php if($i==1) echo "Produk" ?></div>
                          <div class="span9"><?= form_dropdown('produk-'.$i, $produk,  $h->id_produk, 'id="produk-'.$i.'" onchange="cek_stok('.$i.')" required class="chzn-select" data-placeholder="pilih produk"  style="width:220px;" tabindex="2" ') ?>
                            jumlah <input type="text" required="" value="<?= $h->jumlah  ?>" id="jml-<?= $i ?>" name="jml-<?= $i ?>" style="width:50px" />
                            <?php
                            if($i==1){
                              ?>
                            ...
                            echo "<input type='hidden' value='".$h->id."' name='id_detail-".$i."' />";
                           ?>
                          <label style="position:relative;top:-15px" id="stok-<?= $i ?>">stok tersedia : <?= $h->stok ?> </label>
                        </div> 
                      </div>  
                        <?php
                        $i++;
                       ...
                       for ($i; $i<=20 ; $i++) { 
                        ?>
                        <div class="row-fluid" id="produk<?= $i ?>" style="display:none"  >
                          <div class="span3"><?php if($i==1) echo "Produk" ?></div>
                          <div class="span9"><?= form_dropdown('produk-'.$i, $produk,  '', 'id="produk-'.$i.'" onchange="cek_stok('.$i.')" required class="chzn-select" data-placeholder="pilih produk"  style="width:220px;" tabindex="2" ') ?>
                            jumlah <input type="text"  id="jml-<?= $i ?>" name="jml-<?= $i ?>" style="width:50px" />
                            <?php echo "<input type='hidden'   name='id_detail-".$i."' />"; ?>
                       ...
                       ?>                                   
                      <input type="hidden"  name="kode_pengiriman" id="kode_pengiriman" value="<?= $k->kode_pengiriman ?>">     
                      <input type="hidden"  name="jumlah_produk" id="jumlah_produk" value="<?= $j-1; ?>">
                        <br/>                                    
                        <div class="row-fluid">
                           <div class="span3"></div>
  ...
  <script type="text/javascript">
     ...
     $(document).ready(function(){  
       $(".chzn-select").chosen({allow_single_deselect:true}); 
        var i=$("#jumlah_produk").val();
        i++;
        $("#tambah_produk").click(function(){
           //$("#jml-"+i).append(" required='' ");
           $("#produk"+i).show();
           $("#jumlah_produk").val(i);
            i++;
    ...
    // this function will get executed every time the #home element is clicked (or tab-spacebar changed)
        if($(this).is(":checked")) // "this" refers to the element that fired the event
        {
         ...
         $("#form_input_toko").submit(function(){
            ...
            $.ajax({
                        type:'POST',
                        url:'<?= base_url() ?>pengiriman_toko/aksi_edit',
