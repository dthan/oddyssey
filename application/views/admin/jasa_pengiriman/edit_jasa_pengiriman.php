          ...
          <div class="row-fluid">
            <div class="span12">
              ...
              <ul class="breadcrumb br_styled no_space">
                ...
                <li><a href="<?= base_url() ?>jasa_pengiriman">jasa pengiriman</a></li><span class="divider">/</span>
                <li class="active">edit jasa pengiriman</li>
              </ul> 
              <ul class="page_info" >          
                                    
                    <form  method="POST" id="form_input_jasa_pengiriman" action="<?= base_url() ?>jasa_pengiriman/aksi_edit" enctype="multipart/form-data">                     
                    <?php
                     foreach ($jasa_pengiriman as $k) {
                       ?>
                       <div class="row-fluid">
                        <div class="span3">nama jasa pengiriman:</div>
                        <div class="span9">
                          <input required="" id="nama_jasa_pengiriman" type="text" size="40" name="nama_jasa_pengiriman" value="<?= $k->nama_jasa_pengiriman ?>" />*
                          <input type="hidden" name="id" value="<?= $k->id_jasa_pengiriman; ?>">
                        </div> 
                      </div> 
                      <?php
                      if($k->logo!=''){
                        ?>
                         <div class="row-fluid">
                           ...
                           <div class="span9"><img src="<?= base_url()."gambar_jasa_pengiriman/".$k->logo; ?>" width="100"><br/><input type="file" name="gambar"></div> 
                         </div>  
                        <?php
                      }
                      else{
                        ...
                        <div class="row-fluid">
                           <div class="span3">Logo:</div>
                           <div class="span9"><input type="file" name="gambar"></div> 
                         </div>  
                        <?php
                      }
                      ?>
                    ...
                    ?>
                        <input type="hidden" name="id_jasa_pengiriman" value="<?= $k->id_jasa_pengiriman ?>">                                    
                        <div class="row-fluid">
                           <div class="span3"></div>
...
eget, pretium porttitor quam. Sed varius vulputate lacinia. </li>
    ...
    <script type="text/javascript">
     $(document).ready(function(){   
         ...
         $("#kode_jasa_pengiriman").change(function(){
             var kat = $("#kode_jasa_pengiriman").val()
             $.ajax({
                        type:'POST',
                        url:'<?= base_url() ?>jasa_pengiriman/cek_jasa_pengiriman',
                        data: "kode_jasa_pengiriman="+kat,
                        success: function(msg)
                        {
                            if(msg=="ada"){
                              $("#kat_error").fadeIn(100);
                              $("#kode_jasa_pengiriman").val('');
                            }
                            else{
         ...
         /*$("#form_input_jasa_pengiriman").submit(function(){
            //alert("djfkdkfkjdkf");
            $.ajax({
                        type:'POST',
                        url:'<?= base_url() ?>jasa_pengiriman/aksi_edit',
                        data: $("#form_input_jasa_pengiriman").serialize(),
                        success: function(msg)
                        {
                           if(msg=="sukses"){
                              $('#gagal').hide();
