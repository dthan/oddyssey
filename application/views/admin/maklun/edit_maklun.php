          ...
          <div class="row-fluid">
            <div class="span12">
              ...
              <ul class="breadcrumb br_styled no_space">
                ...
                <li><a href="<?= base_url() ?>maklun">maklun</a></li><span class="divider">/</span>
                <li class="active">edit maklun</li>
              </ul> 
              <ul class="page_info" >          
                                    
                    <form  method="POST" id="form_input_maklun" action="<?= base_url() ?>maklun/edit" enctype="multipart/form-data">                     
                    <?php
                     foreach ($maklun as $k) {
                       ?>
                       <div class="row-fluid">
                        <div class="span3">nama maklun:</div>
                        <div class="span9">
                          <input required="" id="nama_maklun" type="text" size="40" name="nama_maklun" value="<?= $k->nama_maklun ?>" />*
                          <i id="kat_error" style="display:none;color:red">kode maklun sudah ada</i>
                        </div> 
                      </div> 
                      <div class="row-fluid">
                        <div class="span3">alamat:</div>
                        <div class="span9"><textarea name="alamat"><?= $k->alamat; ?></textarea></div> 
                      </div>  
                       <?php
                     }
                    ?>
                        <input type="hidden" name="id_maklun" value="<?= $k->id_maklun ?>">                                    
                        <div class="row-fluid">
                           <div class="span3"></div>
...
eget, pretium porttitor quam. Sed varius vulputate lacinia. </li>
    ...
    <script type="text/javascript">
     $(document).ready(function(){   
         ...
         $("#kode_maklun").change(function(){
             var kat = $("#kode_maklun").val()
             $.ajax({
                        type:'POST',
                        url:'<?= base_url() ?>maklun/cek_maklun',
                        data: "kode_maklun="+kat,
         ...
         $("#form_input_maklun").submit(function(){
            //alert("djfkdkfkjdkf");
            $.ajax({
                        type:'POST',
                        url:'<?= base_url() ?>maklun/aksi_edit',
                        data: $("#form_input_maklun").serialize(),
                        success: function(msg)
                        {
                           if(msg=="sukses"){
                              $('#gagal').hide();
